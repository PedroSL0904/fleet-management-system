...
- **Testing:** `pytest` y `pytest-django`
...
3. **Instalar Dependencias e inicializar el entorno:**
   Usa `uv` para instalar todo lo necesario basándose en el `pyproject.toml` (o `uv.lock`):
   ```bash
   uv sync
   ```
...
## 🧪 Pruebas (Testing)
Toda la lógica de negocio (Servicios) y reglas de acceso (RBAC) está cubierta por pruebas automatizadas. Para ejecutarlas:
