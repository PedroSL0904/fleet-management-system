...
def alertas_globales(request: HttpRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        alertas_mant = Mantenimiento.objects.select_related('vehiculo').filter(
            estado__in=['PENDIENTE', 'EN_PROCESO'],
            fecha_servicio__lte=limit_date
        ).exclude(vehiculo__estado='BAJA').order_by('fecha_servicio')

        # 4. Expiring Insurance Policies Query (Ignorando BAJAS)
        alertas_pol = PolizaSeguro.objects.select_related('vehiculo').filter(
            fecha_vencimiento__lte=limit_date
        ).exclude(vehiculo__estado='BAJA').order_by('fecha_vencimiento')
