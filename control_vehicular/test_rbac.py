...
class RBACTestCase(TestCase):
    def setUp(self) -> None:
        ...
        self.mecanico_user = User.objects.create_user(username='mecanico', password='testpass123')
        PerfilUsuario.objects.create(usuario=self.mecanico_user, rol='MECANICO')

        self.chofer_user = User.objects.create_user(username='chofer', password='testpass123')
        PerfilUsuario.objects.create(usuario=self.chofer_user, rol='CHOFER')

        self.user_sin_perfil = User.objects.create_user(username='sinperfil', password='testpass123')

        self.vehiculo = Vehiculo.objects.create(
            placas="TST-100-A", marca="NISSAN", modelo="Versa",
            anio=2024, vin="9HTST1234567890AB",
        )
        self.chofer = Chofer.objects.create(
            nombre="Test", apellidos="Driver", telefono="4420001122",
